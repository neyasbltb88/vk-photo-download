...
function () {
  function HandlersManager() {
    ...
    this.handlers = {
      ...
      closeTimingSettings: function closeTimingSettings() {
        return _this.PhotoDownload.state.settings = 'close_timing';
      },
  ...
  _createClass(HandlersManager, [{
    ...
    value: function setHandlers() {
      ...
      var settings_wrap = wrap.querySelector('.' + this.sel.get('sett.settings_wrap'));
      this.set(settings_wrap, 'closeWatcher', 'transitionend');
      var btn_close = wrap.querySelector('.' + this.sel.get('sett.settings_header_ico'));
    ...
    value: function _startTimer(e) {
      ...
      if (this.PhotoDownload.state.settings == 'close') {
        this.timers.delay = setTimeout(function () {
          _this2.PhotoDownload.state.settings = 'open_timing';
        ...
        setTimeout(function () {
          if (_this2.PhotoDownload.state.settings == 'open') {
            _this2.PhotoDownload.state.settings = 'close_timing';
          }
        }, this.timings.delay);
      }
    }
    ...
    value: function _openSettings() {
      ...
      this.PhotoDownload.wrap.classList.add(this.sel.icon_cog);
      this.PhotoDownload.wrap.querySelector('.cog').classList.add(this.sel.draw_fill, this.sel.draw);
    ...
    key: "_closeTimingSettings",
    value: function _closeTimingSettings() {
      var _this3 = this;

      var set_state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
      ...
      if (set_state) {
        setTimeout(function () {
          _this3.PhotoDownload.state.settings = 'close';
    ...
    value: function set(elem, handler_name) {
      ...
      if (handler_name instanceof Array) {
        handler_name.forEach(function (name) {
          _this4.add(elem, name, event_name);
        });
...
},{"downloadjs":6}],2:[function(require,module,exports){
"use strict";
  ...
  _createClass(PhotoDownload, [{
    ...
    value: function _initWatcher() {
      ...
      types.forEach(function (type) {
        return document.addEventListener(type, _this._watchTrigger.bind(_this));
      });
    ...
    value: function _watchTrigger(event) {
      ...
      triggers.forEach(function (trigger) {
        ...
        var compliance = _this2._checkComplianceTarget(target, trigger); // Если цели события нет в объекте обработчиков, но в обработчике указано, 
        ...
        if (!compliance && trigger.child) {
          ...
          _this2._checkComplianceChild(target, trigger);
        }
      });
    ...
    value: function _saveSettings() {
      var res = this.storage.set('settings', this.settings);
      console.log(res);
    }
  }, {
    key: "_saveState",
    value: function _saveState() {
      var res = this.storage.set('state', this.state);
      console.log(res);
    }
  }, {
    key: "_restoreStorage",
    ...
    value: function init() {
      console.clear();
      console.log('%c%s', window.log_color ? window.log_color.blue : '', 'PhotoDownload: Init'); // Восстанавливаем сохраненное в LocalStorage состояние

      this._restoreStorage(); // Добавляем стили PhotoDownload
...
},{"./handlers":1,"./storage":3,"./templates":4}],3:[function(require,module,exports){
"use strict";
...
},{"./modules/photo-download":2}],6:[function(require,module,exports){
